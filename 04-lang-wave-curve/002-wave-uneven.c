...
// output
char filename[128] = "002-wave.svg\0";
char path[128] = "out/\0";
...
int main(int argc, char **argv){
	...
	for(int i = 0; i < ROUNDS; i+=2){
		for(float a = 0; a < TWOPI; a += TWOPI/divider){
			...
			// wobbleIncr = 1.0;
			float x = width*.5  + SCALE * cos(a) * (i + 2*a/TWOPI + sin(a*wobbleFreq)*wobbleMag*wobbleIncr );
	...
	for(int i = 0; i < ROUNDS; i+=2){
		for(float a = 0; a < TWOPI; a += TWOPI/divider){
			...
			float wobbleIncr = (i + 2*a/TWOPI);
			// wobbleIncr = 1.0;
			float x = width*.5  + SCALE * cos(a-TWOPI*.5) * (i + 2*a/TWOPI - sin(a*wobbleFreq)*wobbleMag*wobbleIncr  ) + SCALE*.67 * one;
