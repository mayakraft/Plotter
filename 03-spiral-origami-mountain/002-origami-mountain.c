...
// output
char filename[128] = "002-mountain.svg\0";
char path[128] = "out/\0";
...
int main(int argc, char **argv){
	...
	for(int i = 1; i < 4; i++){
		...
		for(float a = 0; a < TWOPI; a += TWOPI/circleResolution){
			...
			// if(i < 3) 
				bend3 = 0;
			float x = width*.5  + SCALE * cos(a) * (i + bend1 + bend2 + bend3);// * (i + bend1)+ cos(a) * bend2 - sin(a) * bend3;
			float y = height*.5 + SCALE * sin(a) * (i + bend1 + bend2 + bend3);// * (i + bend1)+ sin(a) * bend2 - cos(a) * bend3;
