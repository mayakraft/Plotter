...
// output
char filename[128] = "002-dual-logarithmic-spiral.svg\0";
char path[128] = "out/\0";
...
int main(int argc, char **argv){
	...
	for(int i = REVOLUTIONS-1; i >= 0; i--){
		for(float a = TWOPI; a >= 0; a -= TWOPI/circleResolution){
			...
			float powRad = powf(i+revolution, 4);
			float radius = SCALE * powRad + sin(a*wobbleFreq) * wobbleMag * powRad;
