...
int main(int argc, char **argv){
	...
	for(int i = 0; i < ROUNDS; i+=2){
		for(float a = 0; a < TWOPI; a += TWOPI/divider){
			x = width*.5  + SCALE * cos(a-TWOPI*.375) * (i + 2*a/TWOPI + cos(a*wobbleFreq)*wobbleMag*(i + 2*a/TWOPI) );
			y = height*.5 + SCALE * sin(a-TWOPI*.375) * (i + 2*a/TWOPI + cos(a*wobbleFreq)*wobbleMag*(i + 2*a/TWOPI) );
			fprintf(file, "%.2f,%.2f ", x, y);
		}
	}
	fprintf(file, "\"/>\n"); // closing quote
