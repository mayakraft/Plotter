...
// output
char filename[128] = "006-nonsymmetrical-sines-wobble.svg\0";
char path[128] = "out/\0";
// path[0] = '\0';
...
// sine curve
float frequency1 = 0.015;
float frequency2 = 0.010;
float frequency3 = 0.0055;
float frequency4 = 0.0045;
	...
	for(int i = 0; i < NUM_GROUPS; i++){
		...
		for(int j = 0; j < GROUP_NUM_VERTICALS; j++){
			...
			for(float h = 0; h < height; h++){
				...
				float wave2 = amplitude2 * sinf(h * frequency2 + .4);
				...
				float x = xGroup + xInternalSpacing + (wave1 + wave2 + wave3 ) * direction * affine + tilt;
				float y = h;
