...
// output
char filename[128] = "004-mountain.svg\0";
char path[128] = "out/\0";
...
int main(int argc, char **argv){
	...
	for(int i = 2; i < ROUNDS; i++){
		...
		for(float a = 0; a < TWOPI; a += TWOPI/circleResolution){
			...
			float x = width*.5  + SCALE * cos(a) * (i + endCap*(masterShape * mW1 + masterShape2 * mW2 + shape2*weight2 + shape3*weight3 + shape4*weight4 ) );
			float y = height*.5 + SCALE * sin(a) * (i + endCap*(masterShape * mW1 + masterShape2 * mW2 + shape2*weight2 + shape3*weight3 + shape4*weight4 ) );
			//  + a/TWOPI 
